...
Tree* add(Tree* const left, Tree* const right) {
    Tree* tmp = new Tree(left, right);
    bool reducing = true;
    while(reducing) reducing = reduce(tmp); // reduce
    return tmp;
}
...
int main(int argc, char const *argv[]) {
    ...
    vector<string> snailfish_nums;
    while(getline(cin,line)) snailfish_nums.push_back(line);
    unsigned long max = 0;
    for(unsigned int i = 0; i < snailfish_nums.size(); ++i) {
        for(unsigned int j = i + 1;  j < snailfish_nums.size(); ++j) {
            Tree* root = add(parse_snailfish_nr(snailfish_nums[i]),parse_snailfish_nr(snailfish_nums[j]));
            unsigned long res = magnitude(root);
            delete root;
            if(res > max) max = res;
        }
    }
